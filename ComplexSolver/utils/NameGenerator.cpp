...
bool ParsedSubname::operator==(const ParsedSubname& other) const = default;
...
bool ParsedName::operator==(const ParsedName& other) const = default;
...
ParsedName NameGenerator::GenerateName() const {
  ...
  const auto smallest_character = std::ranges::min_element(amount_of_uses);
...
ParsedSubname NameGenerator::ParseSubname(const std::string& subname) {
  {
    ...
    const auto position_of_number =
        std::find_if(subname.rbegin(), subname.rend(), [](char c) {
